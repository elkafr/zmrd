...
class HomeProvider extends ChangeNotifier {
  ...
  Future<List<CategoryModel>> getCategoryList1(
    ...
    if (enableSub) {
      response = await _apiProvider
          .get("https://alzumarud.com/api/getcat" + "?api_lang=$_currentLang"+ "&cat_id="+catId);
    } else {
      response = await _apiProvider
          .get("https://alzumarud.com/api/getcat" + "?api_lang=$_currentLang");
    }
  ...
  Future<List<CategoryModel>> getCategoryList2(
    ...
    if (enableSub) {
      response = await _apiProvider
          .get("https://alzumarud.com/api/getcat1" + "?api_lang=$_currentLang"+ "&cat_id="+catId);
    } else {
      response = await _apiProvider
          .get("https://alzumarud.com/api/getcat1" + "?api_lang=$_currentLang");
    }
  ...
  Future<List<City>> getCityList(
    ...
    if (enableCountry) {
      ...
      response = await _apiProvider.get(Urls.CITIES_URL+"?api_lang=$_currentLang");
    }
  ...
  Future<List<Ad>> getAdsListNext($adsId,$adsCat) async {
    final response = await _apiProvider
        .post("https://alzumarud.com/api/next_ads" , body: {
      "ads_id":$adsId,
      "ads_cat":$adsCat,
  ...
  Future<List<Ad>> getAdsListPrev($adsId,$adsCat) async {
    final response = await _apiProvider
        .post("https://alzumarud.com/api/prev_ads" , body: {
      "ads_id":$adsId,
      "ads_cat":$adsCat,
  ...
  Future<List<Ad>> getAdsListCurrent($adsId) async {
    final response = await _apiProvider
        .post("https://alzumarud.com/api/current_ads" , body: {
      "ads_id":$adsId,
    });
    List<Ad> adsList = List<Ad>();
    if (response['response'] == '1') {
